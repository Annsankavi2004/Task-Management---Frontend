...
export default function UserDashboard() {
  ...
  // ===== Sidebar items (member version) =====
  const NAV_ITEMS = [
    { label: "Dashboard", to: "/user", icon: "material-symbols:dashboard" },
    { label: "My Tasks", to: "/my-task", icon: "material-symbols:folder" },
  ];

  // ===== Topbar Search (filters sidebar items) =====
  ...
  // Press "/" to focus the menu search
  useEffect(() => {
    function onKey(e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await axios.get(`${BASE}/tasks/${userId}`);
        const formatted = (res.data || []).map((t) => ({
          ...t,
          created: new Date(t.CreateDate || Date.now())
  ...
  async function handleAddTask(e) {
    ...
    try {
      const res = await axios.post(`${BASE}/create-task`, payload);
      const createdTask = {
        ...res.data,
  ...
  return (
    <div className="ad-wrap">
      ...
      <header className="ad-topbar">
              ...
              {navMatches.length ? (
                navMatches.map((m) => (
                  ...
                  >
                    ...
                    <span>{m.label}</span>
                    <code className="ad-kbd">{m.to.replace("/user", "") || "/"}</code>
                  </li>
                ))
              ) : (
        ...
        {/* 🕒 Current Date & Time (Asia/Colombo) */}
          ...
          <nav className="ad-nav" aria-label="Sidebar">
            {NAV_ITEMS.map((n) => (
              <NavLink key={n.to} to={n.to} className={({ isActive }) => (isActive ? "active" : "")}>
                ...
                {n.label}
                {n.to === "/user" && <Icon icon="material-symbols:arrow-forward" width="16" height="16" />}
              </NavLink>
            ))}
          </nav>
        </aside>
          ...
          <section className="ad-card" id="tasks">
              ...
              {filteredTasks.length === 0 ? (
                ...
                filteredTasks.map((t) => (
                  <div className="ad-tr" role="row" key={t.TaskId}>
                    ...
                    <div role="cell">
                      <span className={`badge ${String(t.Status).replace(/\s/g, "").toLowerCase()}`}>
                        {t.Status}
                      </span>
                    </div>
            ...
            {showForm && (
              <div
                ...
                aria-label="Add task"
                onMouseDown={(e) => {
                  // close when clicking the dim area (but not the form)
